...
fun TextView.setInputTitle(game: Game?) {
    ...
    text = context.getString(com.tobiashehrlein.tobiswizardblock.feature.common.R.string.block_input_general_title, game.currentRoundNumber)
}
...
fun TextView.setPosition(pos: Int?) {
    if (pos == null) return
    text = context.getString(com.tobiashehrlein.tobiswizardblock.feature.common.R.string.block_scores_position, pos)
}
...
fun TextView.setRound(item: SavedGameEntity) {
    text =
        if (item.gameFinished) {
            context.getString(com.tobiashehrlein.tobiswizardblock.feature.common.R.string.saved_game_finished)
        } else {
            context.getString(
